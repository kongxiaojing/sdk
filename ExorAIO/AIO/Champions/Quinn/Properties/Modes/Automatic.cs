    ...
    internal partial class Logics
    {
        ...
        public static void Automatic(EventArgs args)
        {
            ...
            foreach (var target in
                GameObjects.EnemyHeroes.Where(t => t.HasBuff("quinnw") && t.IsValidTarget(Vars.AARange)))
            {
                Variables.Orbwalker.ForceTarget = target;
            }

            if (GameObjects.Player.IsRecalling() || Vars.R.Instance.Name.Equals("QuinnRFinale"))
            {
                return;
            }
                ...
                Vars.Menu["spells"]["w"]["logical"].GetValue<MenuBool>().Value)
            {
                foreach (var enemy in
                    GameObjects.EnemyHeroes.Where(
                        x => !x.IsDead && !x.IsVisible && x.Distance(GameObjects.Player.ServerPosition) < Vars.W.Range))
                {
                    Vars.W.Cast();
                }
